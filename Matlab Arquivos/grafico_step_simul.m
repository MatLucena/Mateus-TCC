...
stairs(t,ye,'LineWidth',1.5)
hold on
stairs(t,ref,'r','LineWidth',1.5)
...
T=title('Resposta à escadaria do modelo ARX1 simulado');
set(gca, 'fontsize', 20, 'Position',[0.155    0.16    0.8    0.75]);
axis([0 40 min(min(ye))-.1 max(max(ye))+.1]);

figure('position', [50, 50, 1200, 500],'Color',[1 1 1])
stairs(t,y2-ref2,'k','LineWidth',1.5)
hold on
...
T=title('Resposta à escadaria do modelo ARX1 simulado');
set(gca, 'fontsize', 20, 'Position',[0.155    0.16    0.8    0.75]);
axis([0 40 min(min(y2-ref2))-.1 max(max(u))+.1]);

...
H=legend('$y_{medido}$','$y_{estimado}$', '$r$', 'Location', 'southeast', 'Orientation', 'vertical');
set(H, 'Interpreter', 'latex', 'Fontsize', 22);
ylabel('Altura (cm)', 'fontsize', 22)
xlabel('Tempo (s)', 'fontsize', 22);
T=title('Resposta ao degrau simulada do modelo SUB1');
set(gca, 'fontsize', 20, 'Position',[0.155    0.16    0.8    0.75]);
axis([0 10 min(min(y))-.1 max(max(y))+.1]);
...
T=title('Resposta ao degrau simulada do modelo SUB1');
set(gca, 'fontsize', 20, 'Position',[0.155    0.16    0.8    0.75]);
axis([0 10 min(min(y2-ref2))-.1 max(max(u))+.1]);

...
\commentib{Acrescente seção falando sobre realimentação de estados e observadr de luenberger contendo inclusive os algoritmos (pseudocódigos) que vc usou para projetar os seus.}
...
\commentib{Falta falar sobre validação de modelos e escolha de estruturas!}

\section{Trabalhos Relacionados}
\cortar{ Nesta subseção falaremos dos trabalhos feitos por outros autores que se relacionam a este.} \commentib{Inicie comentando que modelagem, identificação e controle de sistemas desse tipo estão presentes na literatura. Não faltaram alguns trabalhos?}
...
Em sua dissertação de mestrado Ricco \cite{ricco2012} fala sobre identificação por subespaços com uma linguagem clara e de fácil entendimento. O seu trabalho se relaciona a este devido a identificação por subespaços apesar de não trabalhar com um sistema específico. Ele foi essencial ao entendimento de identificação por subespaços. \commentib{Acho que esse trabalho não é relacionado}
